...
## Как работать
1. Возьми первую задачу из TASKS.md в разделе "В работе"
...
## Если человек сбросил много контекста
Если человек пишет "разбери контекст", "вот материалы проекта", "импортируй данные" или просто вставляет большой текст/ссылки/ТЗ:
1. Не меняй PRD.md, MEMORY.md и TASKS.md напрямую без явной команды.
2. Разбери материалы по разделам:
   - PRD.md: проблема, пользователи, решение, фичи, интеграции, критерии готовности
   - MEMORY.md: финальные решения, ограничения, инфраструктура, правила "нельзя"
   - TASKS.md: задачи в "В работе", "Следующее", "Бэклог", "Выполнено"
   - NOTES.md: вопросы, сомнения, конфликтующие данные, что нужно подтвердить
3. Запиши в NOTES.md предложения в формате [дата] [модель] decision/question.
4. В ответе человеку покажи краткую таблицу: куда какую информацию предлагаешь перенести.
...
## В работе
- [ ] **Первый рабочий результат** — ${answers.discovery ? answers.discovery.mvpFlow : "уточнить первый рабочий шаг и выполнить его"}
      агент: ${agentProfile}
      готово когда: результат можно проверить вручную, а итоги записаны в NOTES.md
...
- [ ] Добавить недостающие проверки после первого результата
...
[2026-05-02] [codex] decision: context import suggests moving target users to PRD.md and deploy limits to MEMORY.md.
...
Если у тебя уже есть материалы проекта, вставь их сразу следующим сообщением:
старые ТЗ, ссылки на сайт, конкурентов, заметки, API, тексты, маркетинг, ограничения.
Попроси агента: "Разбери контекст ниже и предложи, что добавить в PRD.md, MEMORY.md, TASKS.md и NOTES.md".
\`\`\`

## Если нужно сбросить много материалов
Можно отправить агенту ссылки, старые ТЗ, заметки, тексты, требования или описание конкурентов одним сообщением:
...
- какие вопросы оставить в NOTES.md

[вставь материалы проекта]
\`\`\`

Что можно вставлять:
- ссылки на текущий сайт, конкурентов, референсы, GitHub, документацию API
- старые ТЗ, заметки, сообщения, требования, список фич
- описание аудитории, оффера, маркетинга, текстов, воронки
- ограничения по стеку, серверу, доступам, срокам, бюджету
- файлы или фрагменты файлов, которые нужно учесть
...
## Если владелец сбросил много контекста
Если владелец пишет "разбери контекст", "вот материалы проекта", "импортируй данные" или вставляет большой текст/ссылки/ТЗ:
1. Не меняй MEMORY.md, TODO.md, CLAUDE.md, AGENTS.md и docs/PRD.md напрямую без явной команды.
2. Разбери материалы по разделам:
   - docs/PRD.md: проблема, пользователи, решение, фичи, интеграции, критерии готовности
   - MEMORY.md: финальные решения, ограничения, инфраструктура, правила "нельзя"
   - TODO.md: задачи в "В работе", "Следующее", "Бэклог", "Выполнено"
   - .staging/notes.md: вопросы, сомнения, конфликтующие данные, что нужно подтвердить
3. Запиши предложения в .staging/notes.md в формате [дата] [модель] decision/question.
4. В ответе владельцу покажи краткую таблицу: куда какую информацию предлагаешь перенести.
...
Если нужно сбросить агенту старые ТЗ, ссылки, заметки, тексты, требования или материалы конкурентов:

\`\`\`
Разбери контекст ниже.
Не меняй MEMORY.md, TODO.md, ${agentPrimaryFile(agentProfile)} и docs/PRD.md напрямую.
Разложи информацию по разделам:
- что предложить добавить в docs/PRD.md
- что предложить добавить в MEMORY.md
- что предложить добавить в TODO.md
- какие вопросы оставить в .staging/notes.md

[вставь материалы проекта]
\`\`\`

Агент должен записать предложения в .staging/notes.md. Владелец проекта сам переносит подтверждённые пункты в защищённые файлы.
...
# Типы: done / lesson / decision / question
---
[${date}] [agent] decision: context import suggestions go here before owner moves them to MEMORY.md, TODO.md, or docs/PRD.md.
`,
...
Подсказка: файлы будут созданы прямо в текущей папке (${process.cwd()}), без новой подпапки.`
    ...
    if (mode !== "advanced") {
      ...
      console.log("");
      console.log("Если у меня есть материалы проекта, попроси меня прислать их одним сообщением:");
      console.log("ссылки на сайт/конкурентов, старые ТЗ, заметки, API, тексты, маркетинг, ограничения.");
      console.log("Когда я пришлю материалы, разбери их и предложи, что добавить в PRD.md, MEMORY.md, TASKS.md и NOTES.md.");
    } else {
